...
export function useLeafletMap(
  ...
  useLayoutEffect(() => {
    ...
    const map = L.map(container, opts.mapOptions).setView(opts.initialCenter, opts.initialZoom)
    mapRef.current = map
    isInitializedRef.current = true

    const basemap = ELV.vectorBasemapLayer(opts.basemapStyle, opts.basemapOptions)
    basemap.addTo(map)
