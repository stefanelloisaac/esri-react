...
import 'leaflet-draw'
import { createPolygonPopup, createCirclePopup, createPointRow } from './templates'
...
export class MapDrawManager {
  ...
  private attachEventHandlers(): void {
    this.map.on(L.Draw.Event.CREATED, (event: L.LeafletEvent) => {
      ...
      geoJSONLayer.feature.properties.descricaotalhao = this.generateDrawingName()
  ...
  public importGeoJSON(geoJSON: GeoJSON.FeatureCollection, isInitialData: boolean = false): void {
    if (isInitialData) {
  ...
  private formatPointsTable(points: L.LatLng[]): string {
    return points
      .map((point, index) => createPointRow(index + 1, point.lat.toFixed(6), point.lng.toFixed(6)))
      .join('')
  }
